...
static void add_message(ErrorCollector *ec, MessageSeverity severity,
                        ErrorStage stage, SourceLocation loc, const char *fmt,
                        va_list args) {
  if (ec == NULL)
    return;
  ...
  if (ec->count >= ec->capacity) {
    ec->capacity *= 2;
    ec->messages =
        realloc(ec->messages, sizeof(CompilerMessage) * ec->capacity);
  }
...
void add_error(ErrorCollector *ec, ErrorStage stage, SourceLocation loc,
               const char *fmt, ...) {
  va_list args;
  va_start(args, fmt);
...
void vadd_error(ErrorCollector *ec, ErrorStage stage, SourceLocation loc,
                const char *fmt, va_list args) {
  add_message(ec, MSG_ERROR, stage, loc, fmt, args);
}

void add_warning(ErrorCollector *ec, ErrorStage stage, SourceLocation loc,
                 const char *fmt, ...) {
  va_list args;
  va_start(args, fmt);
...
void add_note(ErrorCollector *ec, ErrorStage stage, SourceLocation loc,
              const char *fmt, ...) {
  va_list args;
  va_start(args, fmt);
...
void print_messages(ErrorCollector *ec) {
  ...
  for (int i = 0; i < ec->count; i++) {
    ...
    if (ec->use_color) {
      ...
      if (msg->severity == MSG_ERROR)
        sev_color = ANSI_RED;
      else if (msg->severity == MSG_WARNING)
        sev_color = ANSI_YELLOW;
      else
        sev_color = ANSI_CYAN;
    }
    ...
    if (msg->loc.line > 0) {
      ...
      fprintf(stderr, "%s[%s:%ld:%ld]%s %s:%s%s%s: %s\n", bold,
              msg->loc.filename ? msg->loc.filename : "unknown", msg->loc.line,
              msg->loc.column, reset, stage_name(msg->stage), sev_color,
              sev_label, reset, msg->message);
    } else {
      // no location — omit the prefix
      fprintf(stderr, "%s:%s%s%s: %s\n", stage_name(msg->stage), sev_color,
              sev_label, reset, msg->message);
    }
  }
