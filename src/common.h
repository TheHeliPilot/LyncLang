...
static inline void _stage_error_helper(ErrorCollector *ec, ErrorStage stage,
...
#define stage_fatal(stage, loc, fmt, ...)                                      \
  do {                                                                         \
...
#define stage_trace(stage, fmt, ...)                                           \
  do {                                                                         \
    if (g_trace_mode) {                                                        \
      for (int _i = 0; _i < g_trace_depth; _i++)                               \
        fprintf(stderr, "  ");                                                 \
...
#define stage_trace_enter(stage, fmt, ...)                                     \
  do {                                                                         \
    stage_trace(stage, fmt, ##__VA_ARGS__);                                    \
    g_trace_depth++;                                                           \
  } while (0)

#define stage_trace_exit(stage, fmt, ...)                                      \
  do {                                                                         \
    if (g_trace_depth > 0)                                                     \
      g_trace_depth--;                                                         \
    stage_trace(stage, fmt, ##__VA_ARGS__);                                    \
