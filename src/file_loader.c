...
Program *load_and_parse_file(const char *file_path, int depth) {
  if (depth >= MAX_INCLUDE_DEPTH) {
    stage_error(STAGE_PARSER, NO_LOC,
                "maximum include depth (%d) exceeded — possible circular "
                "include for '%s'",
  ...
  // lex
  TokenList *token_list = tokenize(code, file_path);
  ...
  // parse
  Parser parser = {.tokens = token_list->tokens,
                   .count = token_list->count,
                   .size = token_list->count,
                   .pos = 0};
    ...
    for (int i = 0; i < prog->imports->import_count; i++) {
      ...
      if (!nested) {
        stage_error(STAGE_PARSER, imp->loc,
                    "could not load module '%s' (file: %s)", imp->module_name,
                    nested_path);
        free(nested_path);
        continue;
      }
      ...
      for (int j = 0; j < nested->func_count; j++) {
        ...
        if (imp->type == IMPORT_ALL) {
          ...
          should_include = (strcmp(nested->functions[j]->signature->name,
                                   imp->function_name) == 0);
        }
        ...
        if (should_include) {
          ...
          prog->functions =
              realloc(prog->functions, sizeof(Func *) * new_count);
      ...
      if (imp->type == IMPORT_SPECIFIC) {
        bool found = false;
        for (int j = 0; j < nested->func_count; j++) {
          if (strcmp(nested->functions[j]->signature->name,
                     imp->function_name) == 0) {
            found = true;
            break;
          }
  ...
  for (int i = 0; i < prog->imports->import_count; i++) {
    ...
    for (int j = 0; j < included->func_count; j++) {
      ...
      if (imp->type == IMPORT_ALL) {
        ...
        should_include = (strcmp(included->functions[j]->signature->name,
                                 imp->function_name) == 0);
      }
      ...
      if (should_include) {
        ...
        for (int k = 0; k < prog->func_count; k++) {
          if (strcmp(prog->functions[k]->signature->name,
                     included->functions[j]->signature->name) == 0 &&
              prog->functions[k]->signature->paramNum ==
                  included->functions[j]->signature->paramNum) {
            stage_error(STAGE_PARSER, imp->loc,
    ...
    if (imp->type == IMPORT_SPECIFIC) {
      bool found = false;
      for (int j = 0; j < included->func_count; j++) {
        if (strcmp(included->functions[j]->signature->name,
                   imp->function_name) == 0) {
          found = true;
          break;
        }
