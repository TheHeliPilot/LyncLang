...
int main(int argc, char **argv) {
  ...
  if (!compiler) {
    fprintf(stderr, "Error: no C compiler found. Install gcc, clang, or MSVC "
  ...
  TokenList *token_list = tokenize(code, input_file);
  stage_trace_exit(STAGE_LEXER, "completed, %ld tokens", token_list->count);
  ...
  stage_trace_exit(STAGE_PARSER, "file includes processed, now %d functions",
                   program->func_count);
  ...
  if (opt_level > 0) {
    ...
    if (opt_level >= 1)
      level |= OPT_CONST_FOLD;
    if (opt_level >= 2)
      level |= OPT_DEAD_CODE | OPT_PEEPHOLE;
    if (opt_level >= 3)
      level |= OPT_INLINE;
    if (opt_size) {
