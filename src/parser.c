...
Token *peek(Parser *parser, int offset) {
  ...
  if (parser->pos + offset >= parser->count) {
    Token *lastTok =
        parser->pos > 0 ? &parser->tokens[parser->pos - 1] : &parser->tokens[0];
...
Token *consume(Parser *parser) {
  ...
  stage_trace(STAGE_PARSER, "consume %s (pos=%ld)", token_type_name(tok->type),
              parser->pos - 1);
  return tok;
}
...
ExternBlock *parseExternBlock(Parser *p) {
  ...
  while (peek(p, 0)->type != MORE_T && peek(p, 0)->type != EOF_T) {
    ...
    if (strlen(header) > 0 && t->type != DOT_T)
      strcat(header, ""); // simple concat
    // actually for "sys/types.h" etc we might need better logic, but for
    // "math.h" we likely get VAR_T DOT_T VAR_T lets just append the token text
    ...
    if (t->type == VAR_T) {
      ...
      strcat(header, token_type_name(t->type)); // fallback
  }
  expect(p, MORE_T);
  expect(p, L_BRACE_T);
  ...
  while (peek(p, 0)->type != R_BRACE_T && peek(p, 0)->type != EOF_T) {
    if (peek(p, 0)->type == DEF_KEYWORD_T) {
      ...
      int paramCount = 0;
      FuncParam *params =
          parseFuncParams(p, &paramCount); // reuse existing param parser
      expect(p, R_PAREN_T);
      ...
      if (block->count >= block->capacity) {
        block->capacity *= 2;
        block->signs =
            realloc(block->signs, sizeof(FuncSign *) * block->capacity);
      }
...
Expr *parseFactor(Parser *p) {
  ...
  case MATCH_T: {
    ...
    while (peek(p, 0)->type != R_BRACE_T) {
      ...
      expect(p, SEMICOLON_T);
      branches[count++] =
          (MatchBranchExpr){.pattern = pattern, .caseRet = branchBody};
  ...
  default:
    stage_fatal(STAGE_PARSER, TOK_LOC(tok), "Unexpected token %s in expression",
...
IncludeStmt *parseIncludeStmt(Parser *p) {
  ...
  if (part_count < 2) {
    stage_fatal(STAGE_PARSER, stmt->loc,
                "invalid import: expected 'module.function' or 'module.*' "
                "(e.g., 'std.io.read_int')");
  }
...
Program *parseProgram(Parser *p) {
  ...
  prog->imports->imports =
      malloc(sizeof(IncludeStmt *) * prog->imports->import_capacity);
  ...
  while (peek(p, 0)->type != EOF_T) {
    if (peek(p, 0)->type == INCLUDE_T) {
      if (prog->imports->import_count >= prog->imports->import_capacity) {
        ...
        prog->imports->imports =
            realloc(prog->imports->imports,
                    sizeof(IncludeStmt *) * prog->imports->import_capacity);
      }
      prog->imports->imports[prog->imports->import_count++] =
          parseIncludeStmt(p);
    } else if (peek(p, 0)->type == EXTERN_T) {
      if (prog->ext_block_count >= ext_cap) {
        ext_cap *= 2;
        prog->externBlocks =
            realloc(prog->externBlocks, sizeof(ExternBlock *) * ext_cap);
      }
...
Pattern *parsePattern(Parser *p) {
  ...
  case SOME_T: {
    ...
    assert(bindingTok->value.as_string);
    pattern->as.binding_name = bindingTok->value.as_string;
    break;
  }
  default:
...
Stmt *parseStatement(Parser *p) {
  ...
  stage_trace(STAGE_PARSER, "parse statement starting with %s",
  ...
  case VAR_T: {
    ...
    if (peek(p, 1)->type == COLON_T) {
      ...
      if (peek(p, 0)->type == QUESTION_MARK_T) {
        if (o == OWNERSHIP_NONE)
          stage_fatal(STAGE_PARSER, TOK_LOC(peek(p, 0)),
                      "Non-pointer nullable variable not allowed!");
        consume(p);
        isNullable = true;
      }
      ...
      s->as.expr_stmt = e;
    } else if (peek(p, 1)->type == R_BRACKET_T)
      ; // for = T[expr];
    else
...
Func *makeFunc(char *name, FuncParam *params, int paramCount, TokenType ret,
               Ownership retOwnership, Stmt *body) {
...
bool check_func_sign_unwrapped(FuncSign *a, char *name, int paramNum,
                               Expr **parameters) {
  if (a->paramNum != paramNum) {
    return false;
  }

  for (int i = 0; i < a->paramNum; ++i) {
    if (a->parameters[i].type != parameters[i]->analyzedType) {
      return false;
    }
  }
